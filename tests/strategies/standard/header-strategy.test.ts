...
describe('StandardHeaderConversionStrategy', () => {
  ...
  test('should convert all header levels for traditional BBCode', () => {
    ...
    expect(strategy.convert(`# ${h1Text}`)).toBe(`[size=28][b]${h1Text}[/b][/size]`);
    expect(strategy.convert(`## ${h2Text}`)).toBe(`[size=24][b]${h2Text}[/b][/size]`);
    expect(strategy.convert(`### ${h3Text}`)).toBe(`[size=20][b]${h3Text}[/b][/size]`);
    expect(strategy.convert(`#### ${h4Text}`)).toBe(`[size=18][b]${h4Text}[/b][/size]`);
    expect(strategy.convert(`##### ${h5Text}`)).toBe(`[size=16][b]${h5Text}[/b][/size]`);
    expect(strategy.convert(`###### ${h6Text}`)).toBe(`[size=14][b]${h6Text}[/b][/size]`);
  });
